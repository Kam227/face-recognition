...
It is also worth noting what makes the test set meaningfully different from the validation set beyond just size. During development, every decision — which features to include, what PCA threshold to use, which SVM regularization strength to try — was made while looking at validation accuracy. Even without explicit tuning on the test set, the validation set has implicitly shaped the model. The test set has not. This is why a small accuracy drop from validation to test is expected even for a well-built pipeline, and why the gap should be interpreted as the model's true generalization margin rather than a sign of overfitting.
...
*Haar Cascade detection failures on challenging images.* The Haar Cascade detector works reliably on near-frontal, well-lit faces, but fails on profile views, occluded faces, and low-resolution images. When detection fails, the pipeline falls back to a centre crop of the full 128×128 image, which includes background, clothing, and context that the classifier was never trained to handle. The confusion matrices for LFW show that misclassifications are concentrated on identities whose test images contain atypical poses or lighting — exactly the cases where the detector is least reliable. Identities like Arnold Schwarzenegger and Silvio Berlusconi, who score 0.00 precision and recall in the SVM test results, are particularly affected: their images in LFW frequently include side profiles or partial occlusions that defeat the Haar detector entirely, leading to centre-crop fallbacks that bear little resemblance to the clean frontal crops the classifier was trained on.

*Intra-class variation exceeds the training distribution.* With ≥ 30 images per LFW identity but only roughly 21 in training after the 70% split, the test images for a given person may include poses, accessories (glasses, hats), or lighting conditions not seen during training. HOG and LBP are not invariant to large pose changes; a face turned 30° to the side produces a substantially different HOG descriptor than a frontal view of the same person. This is especially pronounced for subjects whose training images happen to cluster around a narrow range of conditions — the classifier learns a tight feature region for that identity and fails to recognize the same person under different circumstances.
...
*Augmentation of underrepresented classes.* Applying random horizontal flips, brightness jitter, and small affine perturbations to training images for minority classes during the feature extraction phase would synthetically increase per-class sample counts and reduce the dominant-class bias observed in the confusion matrices. This is a low-cost intervention that requires no additional data and works within the existing HOG+LBP framework — it directly addresses the imbalance that causes the classifier to default toward George W. Bush or Colin Powell when uncertain.
