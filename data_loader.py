...
# Official BioID subject boundaries (image indices where a new subject starts).
...
def load_bioid(data_dir: str) -> tuple[list[Path], list[int]]:
    ...
    if not images:
        # Also accept png/jpg in case the user converted them
    ...
    paths, labels = [], []
    for p in images:
        # Extract numeric index from filename, e.g. BioID_0042.pgm → 42
...
def load_lfw(data_dir: str, min_samples: int = LFW_MIN_SAMPLES) -> tuple[list[Path], list[int], list[str]]:
    ...
    # Find person directories (one level or two levels deep)
